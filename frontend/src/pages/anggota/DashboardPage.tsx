...
export default function DashboardPage() {
  ...
  const { data: peminjamanData } = useQuery({
    queryKey: ['peminjaman-me'],
    queryFn: () => peminjamanService.getAll(),
  });

  const allPeminjaman = peminjamanData?.data ?? [];
  const aktif = allPeminjaman.filter((p: any) => p.status === 'DIPINJAM');
  const menunggu = allPeminjaman.filter((p: any) => p.status === 'MENUNGGU');
  const total = allPeminjaman.length;
  const adaDenda = allPeminjaman.some((p: any) => p.denda?.status === 'BELUM_LUNAS');
      ...
      <div className="container mx-auto px-4 max-w-7xl relative z-10">
        ...
        <div className="mb-10 p-6 bg-clay-coffee rounded-2xl flex items-center justify-between shadow-lg">
          ...
          <div className="text-right">
            ...
            <h2 className="text-xl font-inter-bold text-white">{user?.nama ?? '...'}</h2>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6 mb-10">
          ...
          <div className="bg-card border border-border p-6 rounded-2xl flex flex-col justify-between shadow-sm hover:shadow-md transition-shadow relative overflow-hidden group">
            <div className="flex justify-between items-start mb-5 relative z-10">
              ...
              <span className={`w-3 h-3 rounded-full ${aktif.length < 2 ? 'bg-moss-drab' : 'bg-sun-set'}`}></span>
            </div>
            <div className="relative z-10">
              <p className="text-5xl font-inter-bold text-clay-sepia tracking-tighter leading-none mb-1">
                {aktif.length} <span className="text-sm font-inter-bold text-moss-sage tracking-normal">/ 2</span>
              </p>
              <p className="text-xs text-moss-sage font-inter-medium mt-1">Kuota Pinjam</p>
            </div>
          ...
          <div className="bg-card border border-border p-6 rounded-2xl flex flex-col justify-between shadow-sm hover:shadow-md transition-shadow relative overflow-hidden group">
            <h3 className="text-sm font-inter-bold text-clay-coffee uppercase tracking-wider mb-5 relative z-10">Total Riwayat</h3>
            <div className="relative z-10">
              <p className="text-5xl font-inter-bold text-clay-sepia tracking-tighter leading-none mb-1">{total}</p>
              <p className="text-xs text-moss-sage font-inter-medium mt-1">Peminjaman pernah dilakukan</p>
            </div>
          ...
          {/* Card 3: Menunggu */}
          <div className="bg-card border border-border p-6 rounded-2xl flex flex-col justify-between shadow-sm hover:shadow-md transition-shadow relative overflow-hidden group">
            <h3 className="text-sm font-inter-bold text-clay-coffee uppercase tracking-wider mb-5 relative z-10">Menunggu</h3>
            <div className="relative z-10">
              <p className="text-5xl font-inter-bold text-sun-set tracking-tighter leading-none mb-1">
                {menunggu.length} <span className="text-sm font-inter-bold text-sun-burn tracking-normal">Buku</span>
              </p>
          ...
          <div className="bg-card border border-border p-6 rounded-2xl flex flex-col justify-between shadow-sm hover:shadow-md transition-shadow relative overflow-hidden group">
            ...
            <div className="relative z-10">
              <p className={`text-4xl font-inter-bold tracking-tight mb-1 ${!adaDenda ? 'text-moss-drab' : 'text-sun-set'}`}>
                {adaDenda ? 'Ada Denda' : 'Aman'}
              </p>
              <p className="text-xs text-moss-sage font-inter-medium mt-1">
                {adaDenda ? 'Segera lunasi denda kamu' : 'Bebas denda — akses penuh'}
              </p>
            </div>
        ...
        <div className="bg-card border border-border rounded-2xl p-6 shadow-sm">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <Link to="/katalog" className="p-5 bg-[#fdfcfb] border border-border rounded-xl flex items-center gap-4 hover:border-sun-glow hover:bg-sun-glow/5 transition-all shadow-sm">
              ...
              <div>
                ...
                <p className="text-xs text-moss-sage font-inter-medium">Cek katalog digital Perpustakaan.</p>
              </div>
            </Link>
